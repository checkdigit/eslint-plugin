...
describe(ruleId, () => {
  createTester().run(ruleId, rule, {
    ...
    invalid: [
      ...
      //         const keyId = uuid();
      //         const { headers: headers1 } = await fixture.api
      //           .put(\`\${BASE_PATH}/key/\${keyId}?zoneKeyId=\${zoneKeyId}\`)
      //           .set(CREATED_ON_HEADER, createdOn)
      //           .send({
      //             key: '71CA52F757D7C0B45A16C6C04EAFD704',
      ...
      //           .expect(StatusCodes.NO_CONTENT)
      //           .expect(ETAG_HEADER, '1')
      //           .expect(verifyTemporalHeaders);

      //         const updatedOn = new Date().toISOString();
      //         const { headers: headers2 } = await fixture.api
      //           .put(\`\${BASE_PATH}/key/\${keyId}?zoneKeyId=\${zoneKeyId}\`)
      //           .set(IF_MATCH_HEADER, headers1[ETAG_HEADER])
      //           .set(CREATED_ON_HEADER, updatedOn)
      //           .send({
      //             key: '339923C206BA8B19EEBF995DEE6619F7',
      //             checkValue: '1ADEE9',
      //           })
      //           .expect(StatusCodes.NO_CONTENT)
      //           .expect(ETAG_HEADER, '2')
      //           .expect(verifyTemporalHeaders);
